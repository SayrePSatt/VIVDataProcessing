...
bgColor = [255 255 255]/255; %Control the background colors of plots
...
for ii = 3:length(all_files) %Collects the configuration information from the filenames
    temp_config = all_files(ii).name;
    configs(ii-2) = convertCharsToStrings(temp_config(18:25)); %18:28 to also include spring configs
    % distances =
end

%Determine all unique configurations and extract only tests of interest
...
%Extracts relavant confiuration info for data processing
for ii = 1:length(uniq_configs)
    ...
    for kk = 1:length(uniq_red_velo)
        ...
        matching_tests{ii,1}{kk} = filematch(temp_idx);
        %matching test indexing is: {configuration, spring, reduced velocity, matching indicies for each est. red. velo}
    end
end

%% Data Processing
%This is a set of for loops that go thorugh each configuration, spring,
%reduced velocity, and data points to individually process each data point
testing = [];
    ...
    for jj=1:num_spring_configs %Spring Config for each configuration
        ...
        for kk=1:num_red_velo
            ...
            for iii = 1:num_datapoints
                ...
                m = metadata(:,2); %System mass
                f_nw = metadata(:,3); %Wetted natural frequency
                f_na = metadata(:,5); %Vac. natural frequency
                zeta = metadata(:,6); %Damping ratio
                u_red(iii) = U/(f_nw(1)*d_sph(1)); %Calculated reduced velocity from pump freq and nat. freq
                m_star = m(1)/m_d;
                ...
                u_norm(iii) = (u_red(iii)./f_star_peak(iii))*St; %normalized reduced natural frequency
                ...
                %Cycle averaged energy transfer
                ...
                [C_y_phase_alt, C_vortex_phase_alt] = retrievephase2(f_s,f_peak,encoder_filt,C_y,C_vortex); %Unused, cross-correlation technique
                [totalphase, vortexphase] = retrievephase1(encoder_filt,C_y,C_vortex); %Hilbert technique
                pred_phase(iii) = 180-asind((4*pi^3*A_y_star(iii)*mass_damp)/(sqrt(2)*C_y_rms(iii)*(u_red(iii)/f_star_peak(iii))^2*f_star_peak(iii))); %Predict the phase from equation in Assi 2010
            ...
            for kkk = 1:length(results) %Averaged the multiple data points for each configuration and adds bounds
...
%% Phase Subfigure
%Plots the amplitude and phase responses on top of one another to
%investigate the different modes
