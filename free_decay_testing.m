...
for i=1:length(fluid)
    ...
    for ii=1:6
        ...
        for jj=1:length(peak)-m
            time_instance(ii,jj) = (time(peakidx(jj+m))+time(peakidx(jj)))/2;
            f_d(ii,jj) = 1/((time(peakidx(jj+m))-time(peakidx(jj)))/m);
            log_decrement = log(peak(jj)/peak(jj+m))/m;
            zeta(ii,jj) = log_decrement/sqrt(4*pi^2+log_decrement^2);
            f_n(ii,jj) = f_d(ii,jj)/sqrt(1-zeta(ii,jj)^2);
        end
        figure
        plot(time_instance(ii,:),f_n(ii,:))
    end
