...
addpath('E:/EFDL/validation_testing');
mypath = 'E:/EFDL/validation_testing';
...
%% Measuring reference value (taring the linear encoder) 
  ...
  B = importdata([mypath sprintf('/RawData_80mm_1K_sareencode/testVIV_1_00.00.csv', IL(k))], ','); dat0 = B;
  lvdt0 = mean(dat0(:,2));
    
    
%% reading fna,zeta and fnw from the file
% path1= strcat([mypath sprintf('/free_decay/freedecay_air.dat', IL(k))]);
...
% fclose(fileID);
% 
% path2= strcat([mypath sprintf('/free_decay/freedecay_water.dat', IL(k))]);
% fileID = fopen(path2, 'r');
% fn2 = textscan(fileID, '%f%f');
% fnw = fn2{1}; fnw = fnw(1);
% zetawater= fn2{2};zetawater = zetawater(1);

fn1 = table2array(readtable('free_decay/freedecay_air.dat'));
fna = fn1(1);
zetaair = fn1(2);
fn2 = table2array(readtable('free_decay/freedecay_water.dat'));
fnw = fn2(1);
zetawater = fn2(2);
% fclose(fileID);
...
% mass = m{1};
% mbt = m{2};
mass =   1916.2/1000;  %%% NOTE: MASS TO BE CALCULATED BY THE END OF THE EXPERIMENT
mbt = 345.7/1000;
...
datfiles = dir([mypath sprintf('/RawData_80mm_1K_sareencode/*.csv', IL(k))]);
j = 0;
for i = 2:length(datfiles)-1
     name = datfiles(i).name;
    j = j+1;
    if(findstr(name, '00.00'))  % skipping the reference files
        checkname = 1;
    else
        checkname =0;
    end
    ...
    A = importdata([mypath sprintf('/RawData_80mm_1K_sareencode/', IL(k)) name], ','); dat = A;
    t = dat(:,1); % time
    lvdt_raw = dat(:,2); % lvdt signal
