    ...
    for jj=1:num_spring_configs %Spring Config for each configuration
        ...
        for kk=1:num_red_velo
            ...
            for iii = 2 %1:num_datapoints
                ...
                C_y_peaks = findpeaks(C_y(1:end-2000),'MinPeakProminence',0.01);
                C_pot_peaks = findpeaks(C_pot,'MinPeakProminence',0.01);
                C_vortex_peaks = findpeaks(C_vortex,'MinPeakProminence',0.01);

                figure(bif_fig)
                hold on
                scatter(u_red*ones(size(C_y_peaks)),C_y_peaks,20,plotting_color(ii,:),'filled')
